...
function loadQuestion() {
	...
	if (session.filters.length > 0 && session.filters.length < 3) {
		const randomFilter =
			session.filters[Math.floor(Math.random() * session.filters.length)];
	...
	if (session.language.key != "en") {
		...
		if (json.translations[session.language.key] == null) {
			...
			questionField.innerHTML =
				...
				"... " +
				json.translations[session.language.key];
			nextTurn();
		}
	} else {
...
function nextTurn() {
	...
	document
		.getElementById(`jugador${currentPlayer}`)
...
function handleError() {
	questionField.innerHTML =
		"<p class='fw-bold text-danger'>You're going to fast!</p>";
