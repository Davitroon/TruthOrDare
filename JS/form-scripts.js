...
function validateForm() {
	...
	if (
		filterButtons[0].classList.contains("disabled-button") &&
...
function countPlayers(playersNames) {
	...
	document.getElementById(
		"player-names-txt"
	).innerHTML = `Player names (${validPlayers}) <span class="text-danger"> * </span> `;
	return validPlayers;
}
...
document
	...
	.addEventListener("submit", (event) => {
		...
		window.selectedSesion = window.sessions.indexOf(session);
		startGame();
	});
